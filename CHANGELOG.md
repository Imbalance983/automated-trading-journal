...
### Added
- **Cryptocurrency Logos in Trade Cards**
  - 24px circular logos for all crypto assets
  - Integration with CoinCap API for free crypto icons
  - Auto-displays for BTC, ETH, SOL, and all major cryptocurrencies
  - Appears in both open and closed trade lists
  - Graceful fallback if logo fails to load
  - Enhanced visual identification of assets

### Improved
- Trade card UI with flexbox layout
- Better asset identification at a glance
- Professional appearance matching modern crypto apps

### Technical
- `getCryptoLogo()` function extracts base symbol
- Supports USDT, PERP, USD suffix removal
- Error handling with `onerror` attribute
- No API key required (CoinCap public API)
